...
class gradient_descent_one_variable:
    ...
    def __init__(
        self, x_range, y_range, f, dfdx, gd, n_it, lr, x_0, ox_position, t_position
    ):
        x = np.linspace(*x_range, 100)
        ...
        self.path = path(
            self.x_0, self.ax, self.ox_position
    ...
    def update_plot_point(self, firsttime=False):
        ...
        a = self.ax.scatter(
            self.x_0, self.f(self.x_0), marker="o", s=100, color="r", zorder=10
        )
        b = self.ax.scatter(
            self.x_0, self.ox_position, marker="o", s=100, color="k", zorder=10
        )
        c = self.ax.hlines(self.f(self.x_0), 0, self.x_0, lw=2, ls="dotted", color="k")
        d = self.ax.vlines(
            self.x_0, self.ox_position, self.f(self.x_0), lw=2, ls="dotted", color="k"
        )
        t_it = self.ax.annotate(
            f"Iteration #${self.i}$",
            xy=(self.t_position[0], self.t_position[1]),
            xytext=(4, 4),
            textcoords="offset points",
            size=10,
        )
        t_x_0 = self.ax.annotate(
            f"$x_0 = {self.x_0:0.4f}$",
            xy=(self.t_position[0], self.t_position[1] - 1),
            xytext=(4, 4),
            textcoords="offset points",
            size=10,
        )
        t_f = self.ax.annotate(
            f"$f\\,\\left(x_0\\right) = {self.f(self.x_0):0.2f}$",
            xy=(self.t_position[0], self.t_position[1] - 2),
            xytext=(4, 4),
            textcoords="offset points",
            size=10,
        )
        t_dfdx = self.ax.annotate(
            f"$f\\,'\\left(x_0\\right) = {self.dfdx(self.x_0):0.4f}$",
    ...
    def run_gd(self):
        ...
        while (
            self.i <= self.n_it
            and abs(self.dfdx(x_0_new)) >= 0.00001
            and x_0_new >= self.x_range[0]
        ):
            x_0_new = self.gd(self.dfdx, self.x_0, self.lr, 1)
        ...
        if (
            abs(self.dfdx(self.x_0)) >= 0.00001
            or self.x_0 < self.x_range[0]
            or self.x_0 < self.x_range[0]
        ):
            t_res = self.ax.annotate(
                "Has Not Converged",
                xy=(self.t_position[0], self.t_position[1] - 4),
                xytext=(4, 4),
                textcoords="offset points",
                size=10,
            )
        ...
        t_instruction = self.ax.text(
            0.3,
            0.95,
            "[Click on the plot to choose initial point]",
            size=10,
            color="r",
...
class path:
    ...
    def add_path_item(self, x_0, f):
        a = FancyArrowPatch(
            posA=(self.x_0, self.ox_position),
            posB=(x_0, self.ox_position),
            color="r",
...
def dfdx_example_2(x):
    return (np.exp(x) - 1 / x) * np.sin(np.pi * x * 2) + (
        np.exp(x) - np.log(x)
    ) * np.cos(np.pi * x * 2) * 2 * np.pi


# +
def f_example_3(x, y):
    return 85 + 0.1 * (
        -1 / 9 * (x - 6) * x**2 * y**3 + 2 / 3 * (x - 6) * x**2 * y**2
    )
...
def dfdx_example_4(x, y):
    return -(
        -2 * 3 * (x - 0.5) * 10 / (3 + 3 * (x - 0.5) ** 2 + 3 * (y - 0.5) ** 2) ** 2
        + -2 * 2 * (x - 3) * 2 / (1 + 2 * ((x - 3) ** 2) + 2 * (y - 1.5) ** 2) ** 2
        + -2
        * 0.5
        * (x - 3.5)
        * 3
        / (1 + 0.5 * ((x - 3.5) ** 2) + 0.5 * (y - 4) ** 2) ** 2
    )


def dfdy_example_4(x, y):
    return -(
        -2 * 3 * (y - 0.5) * 10 / (3 + 3 * (x - 0.5) ** 2 + 3 * (y - 0.5) ** 2) ** 2
...
def plot_f_cont_and_surf(x_range, y_range, z_range, f, cmap, view):
    ...
    surf = axs.plot_surface(
        X, Y, f(X, Y), cmap=cmap, antialiased=True, cstride=1, rstride=1, alpha=0.69
    )
    axs.set_xlabel("$x$")
    axs.set_ylabel("$y$")
    axs.set_zlabel("$f$")
...
class gradient_descent_two_variables:
    ...
    def __init__(
        ...
        self.update_plot_point(firsttime=True)
        self.path = path_2(
            self.x_0, self.y_0, self.axc, self.axs
        )  # initialize an empty path, avoids existance check
    ...
    def click_plot(self, event):
        """Called when click in plot"""
        if (
            event.xdata <= max(self.x)
            and event.xdata >= min(self.x)
            and event.ydata <= max(self.y)
            and event.ydata >= min(self.y)
        ):
            self.x_0 = event.xdata
    ...
    def update_plot_point(self, firsttime=False):
        ...
        a = self.axc.scatter(
            self.x_0, self.y_0, marker="o", s=100, color="k", zorder=10
        )
        b = self.axc.hlines(
            self.y_0, self.axc.get_xlim()[0], self.x_0, lw=2, ls="dotted", color="k"
        )
        c = self.axc.vlines(
            self.x_0, self.axc.get_ylim()[0], self.y_0, lw=2, ls="dotted", color="k"
        )
        d = self.axs.scatter3D(
            self.x_0, self.y_0, self.f(self.x_0, self.y_0), s=100, color="r", zorder=10
        )
        t_it = self.axs.text(
            self.t_position[0],
            self.t_position[1],
        ...
        t_x_y = self.axs.text(
            self.t_position[0],
            self.t_position[1],
            self.t_position[2] - self.t_space,
            f"$x_0, y_0 = {self.x_0:0.2f}, {self.y_0:0.2f}$",
            size=10,
            zorder=20,
        )
        t_f = self.axs.text(
            self.t_position[0],
            self.t_position[1],
            self.t_position[2] - self.t_space * 2,
            f"$f\\,\\left(x_0, y_0\\right) = {self.f(self.x_0, self.y_0):0.2f}$",
            size=10,
            zorder=20,
        )
        t_dfdx = self.axs.text(
            self.t_position[0],
            self.t_position[1],
            self.t_position[2] - self.t_space * 3,
            f"$f\\,'_x\\left(x_0, y_0\\right) = {self.dfdx(self.x_0, self.y_0):0.2f}$",
            size=10,
            zorder=20,
        )
        t_dfdy = self.axs.text(
            self.t_position[0],
            self.t_position[1],
            self.t_position[2] - self.t_space * 4,
            f"$f\\,'_y\\left(x_0, y_0\\right) = {self.dfdy(self.x_0, self.y_0):0.2f}$",
            size=10,
            zorder=20,
        )
    ...
    def run_gd(self):
        ...
        while (
            self.i <= self.n_it
            and (
                abs(self.dfdx(x_0_new, y_0_new)) >= 0.001
                or abs(self.dfdy(x_0_new, y_0_new)) >= 0.001
            )
            and x_0_new >= self.x_range[0]
            and x_0_new <= self.x_range[1]
            and y_0_new >= self.y_range[0]
            and y_0_new <= self.y_range[1]
        ):
            x_0_new, y_0_new = self.gd(
                self.dfdx, self.dfdy, self.x_0, self.y_0, self.lr, 1
            )
        ...
        if (
            ...
            or abs(self.dfdy(x_0_new, y_0_new)) >= 0.001
            or self.x_0 < self.x_range[0]
            or self.x_0 > self.x_range[1]
            or self.y_0 < self.y_range[0]
            or self.y_0 > self.y_range[1]
        ):
            t_res = self.axs.text(
                self.t_position[0],
                self.t_position[1],
                self.t_position[2] - self.t_space * 5,
                "Has Not Converged",
                size=10,
                zorder=20,
            )
...
class path_2:
    ...
    def add_path_item(self, x_0, y_0, f):
        a = FancyArrowPatch(
            posA=(self.x_0, self.y_0),
            posB=(x_0, y_0),
            color="r",
            arrowstyle="simple, head_width=5, head_length=10, tail_width=1.0",
        )
        b = self.axs.scatter3D(
            self.x_0,
            self.y_0,
            f(self.x_0, self.y_0),
            facecolors="none",
